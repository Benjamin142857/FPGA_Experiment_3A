...
#include "./include/sliding_window_siso.h"
				 ...
				 const mxArray *prhs[] )
{
	...
	float   *output_u_float, *output_c_float, *output_b_float;
	...
	output_b_float = mxGetPr(OUTPUT_B);
	...
	beta = calloc( max_states*(DataLength+KK), sizeof(float) );
