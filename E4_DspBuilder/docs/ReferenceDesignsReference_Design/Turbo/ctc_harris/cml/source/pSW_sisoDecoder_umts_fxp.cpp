/* File pSW_sisoDecoder_umts_fxp.cpp
...
pSW_sisoDecoder_umts_fxp::pSW_sisoDecoder_umts_fxp
(
...
)
{
	...
	sub_frame_size = (int)ceil((double)(frame_size)/num_engines);
	last_window_size = sub_frame_size%sldwin_size;
	num_windows = sub_frame_size/sldwin_size;

	if (last_window_size == 0)
	{
		last_window_size = sldwin_size;
	}
	else
	{
		num_windows++;
	...
	last_engine_frame_size = frame_size - sub_frame_size*(num_engines-1);
	last_engine_last_window_size = last_engine_frame_size%sldwin_size;
	last_engine_num_windows = last_engine_frame_size/sldwin_size;

	if (last_engine_last_window_size == 0)
	{
		last_engine_last_window_size = sldwin_size;
	}
	else
	{
		last_engine_num_windows++;
	}
	...
	metric_c.resize(num_engines);
	beta.resize(num_engines);
	alpha.resize(num_engines);
	alpha_prime.resize(num_engines);
	num_llr_c.resize(num_engines);
	den_llr_c.resize(num_engines);
	num_llr_u.resize(num_engines);
	den_llr_u.resize(num_engines);

	for (int i = 0; i < num_engines; i++)
	{
		metric_c[i].resize(number_symbols,0);
		beta[i].resize((sldwin_size+1)*max_states,0);
...
#ifdef _DEBUG
	if(iteration == 0)
		fout_debug.open("fout_debug_beta.txt", ios_base::out|ios_base::trunc);
	else
		fout_debug.open("fout_debug_beta.txt", ios_base::out|ios_base::app);
#endif
}
...
soft_fix_type pSW_sisoDecoder_umts_fxp::max_star
(
...
void pSW_sisoDecoder_umts_fxp::update_block_beta(int k, int p,int s)
{
...
#ifdef _DEBUG
		if (delta1.quantization_flag())
		{
			quantization_flag = true;
		}
		if (delta1.overflow_flag())
		{
			double alpha_prime_d = beta[p][(s+1)*max_states + state0[ state ]].to_double();
			double metric_c_d = metric_c[p][out0[state]].to_double();
			double delta2_d = alpha_prime_d + metric_c_d;
			overflow_flag = true;
		}

		if (delta2.quantization_flag())
		{
			quantization_flag = true;
		}
		if (delta2.overflow_flag())
		{
			double alpha_prime_d = beta[p][(s+1)*max_states + state1[ state ]].to_double();
			double metric_c_d = metric_c[p][out1[state]].to_double();
			double delta2_d = alpha_prime_d + metric_c_d;
			overflow_flag = true;
		}
#endif
...
/* normalize alpha */
void pSW_sisoDecoder_umts_fxp::update_alpha_prime(int p)
{
	int state;
...
/* normalize beta */
void pSW_sisoDecoder_umts_fxp::normalize_beta(int p, int s)
{
	/* normalize beta */
...
void pSW_sisoDecoder_umts_fxp::update_branch_metric(int k, int p)
{
	soft_fix_type app_in = 0;
...
void pSW_sisoDecoder_umts_fxp::compute_block_alpha(int k, int p)
{
	/* assign inputs */
//	soft_fix_type app_in = 0;
...
//	if (k < frame_size) app_in = input_u_fxp[k];
	...
	for (int state = 0; state < max_states; state++ ) {		
		...
		if (delta1.overflow_flag())
		{
			double alpha_prime_d = alpha_prime[p][rev_state0[state]].to_double();
			double metric_c_d = metric_c[p][rev_out0[state]].to_double();
			double delta2_d = alpha_prime_d + metric_c_d;
			overflow_flag = true;
		}
		...
		if (delta2.overflow_flag())
		{
			double alpha_prime_d = alpha_prime[p][rev_state1[state]].to_double();
			double metric_c_d = metric_c[p][rev_out1[state]].to_double();
			double delta2_d = alpha_prime_d + metric_c_d;
...
void pSW_sisoDecoder_umts_fxp::compute_llr(int k,int p, int s)
{
	int state;
...
#ifdef _DEBUG
		if (delta_den[state].quantization_flag())
		{
			quantization_flag = true;
		}
		if (delta_den[state].overflow_flag())
		{
			overflow_flag = true;
		}

		if (delta_num[state].quantization_flag())
		{
			quantization_flag = true;
		}
		if (delta_num[state].overflow_flag())
		{
			overflow_flag = true;
		}
#endif
...
void pSW_sisoDecoder_umts_fxp::compute_output(int k,int p)
{
	soft_fix_type temp;
...
#ifdef _DEBUG
		if ((*output_llr_fxp_p)[k].quantization_flag())
		{
			quantization_flag = true;
		}
		if ((*output_llr_fxp_p)[k].overflow_flag())
		{
			overflow_flag = true;
		}
		if (temp.quantization_flag())
		{
			quantization_flag = true;
		}
		if (temp.overflow_flag())
		{
			overflow_flag = true;
		}
#endif
...
void pSW_sisoDecoder_umts_fxp::compute_block_beta(int k,int p,int s)
{
	/* precompute all possible branch metrics */
...
void pSW_sisoDecoder_umts_fxp::update_beta_at_the_tail()
{
	for (int state = 0; state < max_states; state++)
	{
	...
	int alpha_offset = num_engines*max_states;
	int offset = alpha_offset + (num_windows*(num_engines-1) + last_engine_num_windows - 1)*max_states;
...
void pSW_sisoDecoder_umts_fxp::update_alpha_beta_prev_for_next_iteration(int window_id)
{
	int p, offset, state;
	...
	// save alpha
	if(window_id == num_windows - 1) // last window in a parallel processor (but not the last processor)
	{
		for(p = 0; p < num_engines-1; p++)
		{
	...
	if (window_id >= last_engine_num_windows)
	{
		return;
	}
...
void pSW_sisoDecoder_umts_fxp::compute_beta(int window_id)
{
	int s, k, p;
	...
	for(p = 0; p < num_engines-1; p++)
	{
		for (s = end - 1; s >= 0; s--)
		{
			k = p * sub_frame_size + window_id * sldwin_size + s;
	...
	if (window_id == last_engine_num_windows - 1)
	{
		end = last_engine_last_window_size;
	}
	else
	{
		end = sldwin_size;
...
void pSW_sisoDecoder_umts_fxp::compute_alpha_and_llr(int window_id)
{
	int s, k, p;
	int end = sldwin_size;
...
void pSW_sisoDecoder_umts_fxp::initialize_alpha_at_iteration_0()
{
	for (int p = 0; p < num_engines; p++)
	{
...
void pSW_sisoDecoder_umts_fxp::initialize_alpha()
{
	for (int p = 0; p < num_engines; p++)
	{
		int offset = p * max_states;
...
void pSW_sisoDecoder_umts_fxp::initialize_beta_at_iteration_0
(
)
{
	int alpha_offset = num_engines*max_states;
	...
	// last engine
	int p = num_engines-1;
	for (int window_id = 0; window_id < last_engine_num_windows; window_id++)
	{
		offset = alpha_offset + (window_id + p * num_windows) * max_states;

		for (int state = 0; state < max_states; state++)
		{
			if (window_id == last_engine_num_windows - 1) // the end of the trellis diagram
			{
				// Initialize end beta
				alpha_beta_prev_fxp[offset + state] = (state == 0 ? (soft_fix_type)0 : MIN_LOG);
			}
...
/* initialize beta */
void pSW_sisoDecoder_umts_fxp::initialize_beta
(
	int window_id
)
{
	...
	for (int p = 0; p < num_engines-1; p++)
	{
		...
		// the block size may be less than sldwin_size for the last window
		if(window_id == num_windows - 1) 
		{
			end = last_window_size;
		}
...
#ifdef _DEBUG
void pSW_sisoDecoder_umts_fxp::dump_alpha_beta_prev(int window_id)
{
	int p, state, s, k, start;

	for(p = 0; p < num_engines; p++)
	{
		fout_debug << "iteration = " << iteration << ", p = " << p << ", start = " << window_id * sldwin_size + p * sub_frame_size << endl;
		for (state = 0; state < max_states; state++)
		{
...
//		int window_id = start/sldwin_size + p;
...
void pSW_sisoDecoder_umts_fxp::do_window(int window_id)
{
...
void pSW_sisoDecoder_umts_fxp::do_work()
{
	if(iteration <= 1)
	{
		/* Initalization */
	...
	for(int w = 0; w < num_engines + num_windows*(num_engines-1) + last_engine_num_windows; w++)
	{
