...
pSW_sisoDecoder_umts::pSW_sisoDecoder_umts
(
	double  *output_u,
	double  *output_c,
	double  *output_b,
...
: debug(false)
{
	...
	sub_frame_size = frame_size/num_engines;
	...
	{
		num_windows = sub_frame_size/sldwin_size + 1;
	}
...
double pSW_sisoDecoder_umts::max_star
(
	double delta1,
	double delta2
)
...
void pSW_sisoDecoder_umts::update_block_beta(int k, int p, int s)
{
...
/* normalize alpha */
void pSW_sisoDecoder_umts::update_alpha_prime(int p)
{
	int state;
...
/* normalize beta */
void pSW_sisoDecoder_umts::normalize_beta(int p, int s)
{
	/* normalize beta */
...
void pSW_sisoDecoder_umts::update_branch_metric(int k, int p)
{
...
void pSW_sisoDecoder_umts::compute_block_alpha(int k, int p)
{
	/* assign inputs */
	float app_in = (k < frame_size) ? input_u[k] : 0;
...
void pSW_sisoDecoder_umts::compute_llr(int k,int p, int s)
{
	int state;
...
void pSW_sisoDecoder_umts::compute_output(int k,int p)
{
	if (k < frame_size)
	{
...
void pSW_sisoDecoder_umts::compute_block_beta(int k,int p,int s)
{
	/* precompute all possible branch metrics */
...
void pSW_sisoDecoder_umts::update_beta_at_the_tail()
{
	for (int state = 0; state < max_states; state++)
	{
	...
	int offset = alpha_offset + (num_engines * num_windows - 1)*max_states;
...
void pSW_sisoDecoder_umts::update_alpha_beta_prev_for_next_iteration(int window_id)
{
	int p, offset, state;
	...
	{
		if(window_id == num_windows - 1 && p < num_engines -1) // last window in a parallel processor (but not the last processor)
		{
...
void pSW_sisoDecoder_umts::compute_beta(int window_id)
{
	int s, k, p;
...
void pSW_sisoDecoder_umts::compute_alpha_and_llr(int window_id)
{
	int s, k, p;
	int state;
	int end = sldwin_size;
...
void pSW_sisoDecoder_umts::initialize_alpha_at_iteration_0()
{
...
void pSW_sisoDecoder_umts::initialize_alpha()
{
	for (int p = 0; p < num_engines; p++)
	{
		int offset = p * max_states;
...
void pSW_sisoDecoder_umts::initialize_beta_at_iteration_0
(
)
{
	int alpha_offset = num_engines*max_states;
	...
	for (int p = 0; p < num_engines; p++)
	{
		for (int window_id = 0; window_id < num_windows; window_id++)
		{
			...
			for (int state = 0; state < max_states; state++)
			{
				if (p == num_engines-1 && window_id == num_windows - 1) // the end of the trellis diagram
				{
					// Initialize end beta
					alpha_beta_prev[offset + state] = (state == 0 ? 0 : -MAXLOG);
				}
...
/* initialize beta */
void pSW_sisoDecoder_umts::initialize_beta
(
	int window_id
)
{
...
void pSW_sisoDecoder_umts::do_window(int window_id)
{
...
void pSW_sisoDecoder_umts::do_work()
{
	if(iteration <= 1)
	{
		/* Initalization */
	...
	for(int w = 0; w < (1 + num_windows) * num_engines; w++)
	{
