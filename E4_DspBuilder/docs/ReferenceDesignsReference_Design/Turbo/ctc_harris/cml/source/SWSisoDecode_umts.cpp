/* file: SWSisoDecoder_umts.c

   Description: Soft-in/soft-out decoding algorithm for a convolutional code

   The calling syntax is:

      [output_u, output_c] = SWSisoDecoder_umts(input_u, input_c, g_encoder, [code_type], [dec_type] )
   ...
   Function SWSisoDecoder_umts is part of the Iterative Solutions 
...
#include "pSW_sisoDecoder_umts.h"
				 ...
				 const mxArray *prhs[] )
{
	...
	if (nrhs < 3 ) {
		mexErrMsgTxt("Usage: [output_u, output_c] = SWSisoDecoder_umts(input_u, input_c, g_encoder, code_type, decoder_type )");
	} else {
		...
		if ( CodeLength != nn*(DataLength+mm) ) 
			mexErrMsgTxt( "SWSisoDecoder_umts: Length of input_u and input_c don't agree" );
...
//					g_encoder[i] = g_encoder[i] + (int) pow(2,(KK-j-1)); 
	...
	if (nlhs  != 3 || nrhs < 8) {
		mexErrMsgTxt("Usage: [output_u, output_c, output_beta] = SWSisoDecoder_umts(input_u, input_c, g_encoder, code_type, decoder_type, num_engines, sldwin_size, beta )" );
	} 
...
//		input_u_float, input_c_float, KK, nn, DataLength, dec_type, num_engines, sldwin_size, beta );

	pSW_sisoDecoder_umts siso ( output_u_float, output_c_float, output_b_float, out0, state0, out1, state1,
		input_u_float, input_c_float, KK, nn, DataLength, dec_type, num_engines, sldwin_size, beta, iteration);
