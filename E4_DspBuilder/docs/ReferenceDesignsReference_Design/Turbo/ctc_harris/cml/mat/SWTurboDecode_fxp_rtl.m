...
%        input_decoder_c = reshape(eval(sprintf('ctc_input%d',K_interleaver))',1,[]) ./ (2^bwr);
    ...
    if (dump_input == 1) 
        ...
        cml_save_to_file(reshaped_input', [dump_dir '/ctc_data_input.txt'], bwr);
        cml_save_to_file(K_interleaver + 4, [dump_dir '/ctc_blksize.txt'], 0);
        cml_save_to_file(turbo_iterations*2, [dump_dir '/ctc_iter_data.txt'],  0);       
%	    save 'input.mat' data input_upper_u input_decoder_c;
	end
