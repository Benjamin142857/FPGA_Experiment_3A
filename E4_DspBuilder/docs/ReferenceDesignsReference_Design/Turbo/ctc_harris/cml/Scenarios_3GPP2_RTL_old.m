...
for record = 40:20:5144 %reduce testdata set by factor of 20
	sim_param(record).comment = sprintf('RTL 3GPP2 test: %d bits, s-maxlogMAP/8it/8bits', record);
	...
	sim_param(record).code_configuration = 5;
	...
	sim_param(record).code_interleaver = ...
	    strcat( 'Create3GPP2Interleaver(', int2str(sim_param(record).framesize ), ')' );
