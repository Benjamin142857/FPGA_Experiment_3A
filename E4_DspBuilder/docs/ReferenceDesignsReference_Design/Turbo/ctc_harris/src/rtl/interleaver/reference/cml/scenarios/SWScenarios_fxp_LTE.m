...
sim_param(record).fxp_data_width = [6 2 9 2];
...
sim_param(record).filename = strcat( data_directory, ...
    strcat( 'fxp',int2str(record),  int2str(sim_param(record).framesize ), sim_param(record).channel, int2str( sim_param(record).num_subblocks ), int2str( sim_param(record).subblock_size ),'_',sprintf('%d',sim_param(record).fxp_data_width(1:end)),int2str(sim_param(record).decoder_type),'.mat') );
