...
#my @umts_blocks = ( 40 .. 2047 );
    ...
    my @umts_blocks_mid = ( 2001 .. 2500); # 3999);
...
sub generate_random_test_cases()
{
	...
	{
        push(@{$records{$test_index}}, @blocks[ map { (rand @blocks) x (1+rand($num_of_duplicates)) } ( 1 .. $num_of_blocks )] );
        $test_index++;
	}
}
...
{
    my @blocks_to_sweep = (@umts_blocks_mid);
