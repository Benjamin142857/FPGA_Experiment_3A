...
qh_matrix = reshape(qh, R, engine_size_c);
qh_matrix = qh_matrix.';
% generate input sine wave for testing
fid = fopen('sine_interpolation_fir_input.txt','wt');
fc = 0.2;
fo = fc/2; % half of cutoff freq.
t = 0:1/fc/20:1600;
x = cos(2*pi*fo*t);
qx_tmp = floor(x*power(2, din_width_c-2))-1;
% convert input data into c0, c1, c2, c3, X, X, X, .... format
qx = [];
p = 16; % input period
for n = 1:floor(length(qx_tmp)/4)
    a = qx_tmp( (n-1)*chan+1:n*chan);
    b = [a zeros(1, p-chan)];
    qx = [qx b];
end;
for k=1:length(qx)
...
% define file
fid = fopen('template_interpolation_fir.vhd','wt');

% print filter coeffi
fprintf(fid, 'constant coeff_matrix_c : coeff_matrix_t := (');
